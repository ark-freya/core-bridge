                    ...
                    switch (nesMessage.type) {
                        ...
                        case "request": {
                            ...
                            if (nesMessage.statusCode === 200 && nesMessage.payload) {
                                ...
                                scObject = { rid: nesMessage.id, error: { message: JSON.parse(errorMessage).message } };
                            }
                    ...
                    try {
                        ...
                        if (event && (event.startsWith("p2p.") || event === "#handshake")) {
                            ...
                            this.socket._timers[cid] = setTimeout(() => {
                                if (this.socket && this.socket._ids) {
                                    delete this.socket._ids[cid];
                                }
                                if (this.socket && this.socket._timers) {
                                    delete this.socket._timers[cid];
                                }
                            }, 30000);
                        }
                    } catch {
